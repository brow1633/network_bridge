...
void UdpInterface::load_parameters()
{
    ...
    RCLCPP_INFO(node_->get_logger(), "Local Address: %s", local_address_.c_str());
    RCLCPP_INFO(node_->get_logger(), "Receive Port: %d", receive_port_);
    RCLCPP_INFO(node_->get_logger(), "Remote Address: %s", remote_address_.c_str());
    RCLCPP_INFO(node_->get_logger(), "Send Port: %d", send_port_);
}
...
void UdpInterface::setup_udp()
{
    ...
    send_socket_.open(ip::udp::v4(), ec);
    error_handler(ec, "Failed to open sending socket", fatal);

    RCLCPP_INFO(node_->get_logger(), "Setting up remote endpoint with address: %s and port: %u", remote_address_.c_str(), send_port_);

    send_endpoint_ = ip::udp::endpoint(ip::address::from_string(remote_address_, ec), send_port_);
    error_handler(ec, "Failed to parse remote address", fatal);

    RCLCPP_INFO(node_->get_logger(), "Established connection to send data to: %s:%u", remote_address_.c_str(), send_port_);

    // Setup receiving socket
    receive_socket_.open(ip::udp::v4(), ec);
    error_handler(ec, "Failed to open receiving socket", fatal);

    RCLCPP_INFO(node_->get_logger(), "Setting up local endpoint with address: %s and port: %u", local_address_.c_str(), receive_port_);
    receive_endpoint_ = ip::udp::endpoint(ip::address::from_string(local_address_, ec), receive_port_);
...
void UdpInterface::error_handler(const boost::system::error_code& ec, const std::string & error_message, bool fatal)
{
    if(ec)
    {
...
void UdpInterface::start_receive()
{
    receive_socket_.async_receive_from(
        buffer(receive_buffer_), receive_endpoint_,
        boost::bind(&UdpInterface::receive, this, placeholders::error, placeholders::bytes_transferred));
}

void UdpInterface::receive(const boost::system::error_code& ec, std::size_t bytes_recvd)
{
    if(bytes_recvd <= 0)
    {
        start_receive();
...
void UdpInterface::write(const std::vector<uint8_t>& data)
{
    ...
    boost::system::error_code ec;
    send_socket_.send_to(buffer(data), send_endpoint_, 0, ec);
